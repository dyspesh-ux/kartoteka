...
порты. Вход: `Administrator` / `admin`. Модуль Access Registry — в разделе рабочих пространств или
через поиск («HR Source», «Sync Log»).
...
В интерфейсе создайте **HR Source**: код `ZUP1`, базовый URL `http://127.0.0.1:8765/hs/hr_export`,
пользователь `svc_hr_export`, пароль `secret`. Нажмите **«Синхронизировать сейчас»**, результат
появится в **Sync Log**.
